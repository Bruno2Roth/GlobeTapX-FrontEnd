...
function sanitizeHtml(value) {
  ...
  const allowedTags = new Set(["A", "B", "BR", "CODE", "EM", "H2", "H3", "H4", "I", "LI", "OL", "P", "PRE", "STRONG", "UL"]);
...
function ImagePlaceholder({ name }) {
  return <div className="country-documentation__image-placeholder" role="img" aria-label={`Sin imagen de ${name || "país"}`}><span aria-hidden="true">🌐</span><span>Imagen no disponible</span></div>;
}
...
export default function DocumentacionPais({ paisId }) {
  ...
  if (paisId === undefined || paisId === null || paisId === "") return <p className="country-documentation__state">No hay un país actual configurado en tu perfil.</p>;
  if (requestState.paisId !== String(paisId)) return <Loader />;
  if (requestState.status === "not-found" || requestState.status === "error") return <p className="country-documentation__state country-documentation__state--error">{requestState.error}</p>;
  ...
  const sanitizedDocumentation = documentationIsHtml ? sanitizeHtml(data.documentacion) : "";

  return <article className="country-documentation" aria-live="polite">
    <div className="country-documentation__image-wrap">{hasImage ? <img className="country-documentation__image" src={data.imagen} alt={data.paisNombre} onError={() => setImageFailed(true)} /> : <ImagePlaceholder name={data.paisNombre} />}</div>
    <div className="country-documentation__body">
      ...
      <div className={`country-documentation__content${hasDocumentation ? "" : " country-documentation__content--empty"}`}>
        {hasDocumentation ? (documentationIsHtml ? <div dangerouslySetInnerHTML={{ __html: sanitizedDocumentation }} /> : <p className="country-documentation__plain-text">{data.documentacion}</p>) : <p>{EMPTY_DOCUMENTATION}</p>}
      </div>
    </div>
  </article>;
}
